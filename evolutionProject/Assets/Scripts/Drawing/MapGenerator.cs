...
public class MapGenerator : MonoBehaviour
{
	...
	void HandleBrush()
	{
		...
		// Calculate brushCellRadius as before
		float cellSize = mapWidth / (numChunksX * (chunkResolution - 1));
		float brushCellRadius = brushWorldRadius / cellSize;

		Vector2 mousePos = InputHelper.MouseWorldPos;
		brush.position = new Vector3(mousePos.x, mousePos.y, -1);

		// Fine-tune the sprite size using a scaling factor
		float spriteScaleFactor = 0.03f; // Adjust this factor as needed
		float spriteScale = brushCellRadius * 2 * spriteScaleFactor;
		brush.localScale = Vector3.one * spriteScale;

		// Update the brush collider's radius based on brushCellRadius (if applicable)
		//CircleCollider2D circleCollider = brush.GetComponent<CircleCollider2D>();
		//if (circleCollider != null)
		//{
		//	circleCollider.radius = brushCellRadius * spriteScaleFactor;
		//}
	...
	void EraseBrushStroke()
	{
		...
		bool subtractBrush = InputHelper.AnyOfTheseKeysHeld(KeyCode.LeftAlt, KeyCode.LeftShift);
		float brushValue = brushStrength * Time.deltaTime * ((subtractBrush) ? -1 : 1); // Negative value for erasing
		...
		// Erase brush at n points along stroke path
		for (int i = 0; i < strokePath.Length; i++)
		{
			int brushCentreX = strokePath[i].x;
			int brushCentreY = strokePath[i].y;
	...
	void Generate()
	{
		...
		Vector2 center = new Vector2(totalNumCellsX / 2f, totalNumCellsY / 2f);
		...
		for (int y = 0; y < totalNumCellsY; y++)
		{
			for (int x = 0; x < totalNumCellsX; x++)
			{
				...
				// Adjust the parameters here to control the empty space
				// You can modify the condition to create a larger empty space
				if (isBorder && border)
				{
				...
				else
				{
					...
					float distance = Vector2.Distance(center, cellPosition);
